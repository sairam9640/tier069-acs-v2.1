...
function checkRateLimit(key) {
  ...
  if (entry.lockedUntil && now < entry.lockedUntil) {
    ...
    return {
      allowed: false,
      message: `Account temporarily locked due to repeated failed attempts. Please try again in ${remainingMins} minute(s).`
    };
  }
  ...
  if (entry.count >= MAX_FAILED_ATTEMPTS) {
    ...
    return {
      allowed: false,
      message: `Too many failed login attempts. Account locked for 15 minutes for your protection.`
    };
  }
...
async function loginSuperAdmin(emailInput, clientIp = '127.0.0.1', userAgent = 'UNKNOWN') {
  ...
  const rateKey = `SA_REQ_${clientIp}_${cleanEmail}`;
...
async function loginOperator(phoneOrUser, password = null, requestedSlug = null, clientIp = '127.0.0.1', userAgent = 'UNKNOWN') {
  ...
  const rateKey = `OP_${clientIp}_${uCleanDigits || u}`;
  ...
  if (requestedSlug && requestedSlug !== 'all' && requestedSlug !== 'default') {
  ...
  if (!tenant) {
    tenant = allTenants.find(t => {
      ...
      return (tPhone && uCleanDigits && (tPhone === uCleanDigits || tPhone.endsWith(uCleanDigits) || uCleanDigits.endsWith(tPhone))) ||
             (tUser && tUser === u) ||
             (tSlug && tSlug === u) ||
             (tEmail && tEmail === u);
    });
  ...
  const targetPhone = tenant.phone || uCleanDigits;
  ...
  // Dispatch via Central Super Admin WhatsApp Web Gateway
  const waSent = await whatsappService.sendOperatorLoginOtp(targetPhone, otp, tenant.name);

  if (!waSent) {
    // Check if any WhatsApp session is connected
    let hasConnectedSession = false;
    for (const session of whatsappService.activeSessions.values()) {
    ...
    await db.addAuditLog({
      event: 'OPERATOR_OTP_WHATSAPP_DISPATCH_FAILED',
      phone: targetPhone,
      tenantSlug: tenant.slug,
      ip: clientIp,
      userAgent,
      status: 'FAILED',
      details: { reason: hasConnectedSession ? 'WhatsApp message delivery failed' : 'Master WhatsApp Gateway is disconnected' }
    });

    return {
      status: 503,
      success: false,
      message: 'WhatsApp OTP could not be sent because WhatsApp Web Gateway is not linked or currently offline. Please contact your ACS Super Admin / Service Provider to reconnect WhatsApp.'
    };
  }

  const maskedPhone = targetPhone.length >= 10 ? targetPhone.replace(/(\d{2})(\d{6})(\d{2})/, '$1******$3') : targetPhone;
  ...
  return {
    ...
    tenantSlug: tenant.slug,
    message: `6-Digit OTP code sent to your registered WhatsApp (+91 ${maskedPhone}). Valid for 5 minutes.`
  };
}
