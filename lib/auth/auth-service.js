...
async function loginOperator(phoneOrUser, password = null, requestedSlug = null, clientIp = '127.0.0.1', userAgent = 'UNKNOWN') {
  ...
  if (!tenant) {
    tenant = allTenants.find(t => {
      ...
      const t10 = rawPhone.slice(-10);
      const tUser = (t.username || '').trim().toLowerCase();
      const tUserDigits = tUser.replace(/\D/g, '').slice(-10);
      const tSlug = (t.slug || '').trim().toLowerCase();
      const tEmail = (t.email || '').trim().toLowerCase();

      return (t10 && u10 && t10 === u10) ||
             (rawPhone && uDigits && (rawPhone === uDigits || rawPhone.endsWith(uDigits) || uDigits.endsWith(rawPhone))) ||
             (tUser && (tUser === uClean || (tUserDigits && u10 && tUserDigits === u10))) ||
             (tSlug && tSlug === uClean) ||
             (tEmail && tEmail === uClean);
    });
  }
