...
// 2.1 OTP DISPATCH RATE-LIMITING & ANTI-SPAM PROTECTION
const otpDispatchRecords = new Map(); // identifier -> { count, firstDispatch, lastDispatch }
const MAX_OTP_DISPATCH_PER_WINDOW = 3;
const OTP_WINDOW_MS = 15 * 60 * 1000; // 15 mins window
const OTP_COOLDOWN_MS = 60 * 1000; // 60s cooldown

function checkOtpDispatchThrottle(identifier) {
  const now = Date.now();
  const entry = otpDispatchRecords.get(identifier);
  if (!entry) return { allowed: true };

  // 1. Minimum 60-second cooldown
  if (now - entry.lastDispatch < OTP_COOLDOWN_MS) {
    const waitSec = Math.ceil((OTP_COOLDOWN_MS - (now - entry.lastDispatch)) / 1000);
    return {
      allowed: false,
      status: 429,
      message: `Please wait ${waitSec} second(s) before requesting another OTP verification code.`
    };
  }

  // 2. Window reset
  if (now - entry.firstDispatch > OTP_WINDOW_MS) {
    otpDispatchRecords.delete(identifier);
    return { allowed: true };
  }

  // 3. Max 3 OTP dispatches per 15-minute window
  if (entry.count >= MAX_OTP_DISPATCH_PER_WINDOW) {
    const waitMin = Math.ceil((OTP_WINDOW_MS - (now - entry.firstDispatch)) / 60000);
    return {
      allowed: false,
      status: 429,
      message: `Maximum OTP request limit reached (3 per 15 mins). Please wait ${waitMin} minute(s).`
    };
  }
...
function recordOtpDispatch(identifier) {
  const now = Date.now();
  let entry = otpDispatchRecords.get(identifier);
  if (!entry || now - entry.firstDispatch > OTP_WINDOW_MS) {
    entry = { count: 1, firstDispatch: now, lastDispatch: now };
  } else {
    entry.count += 1;
    entry.lastDispatch = now;
  }
  otpDispatchRecords.set(identifier, entry);
}
...
async function loginSuperAdmin(emailInput, clientIp = '127.0.0.1', userAgent = 'UNKNOWN') {
  ...
  // Anti-Spam / OTP Dispatch Rate-Limiter Check (Max 3 per 15 mins + 60s cooldown)
  const otpThrottleCheck = checkOtpDispatchThrottle(cleanEmail);
  if (!otpThrottleCheck.allowed) {
    return { status: otpThrottleCheck.status, success: false, message: otpThrottleCheck.message };
  }

  recordOtpDispatch(cleanEmail);
  clearDualFailedAttempts(clientIp, cleanEmail);
...
async function loginOperator(phoneOrUser, password = null, requestedSlug = null, clientIp = '127.0.0.1', userAgent = 'UNKNOWN') {
  ...
  // Anti-Spam / OTP Dispatch Rate-Limiter Check (Max 3 per 15 mins + 60s cooldown)
  const otpThrottleCheck = checkOtpDispatchThrottle(targetPhone);
  if (!otpThrottleCheck.allowed) {
    return { status: otpThrottleCheck.status, success: false, message: otpThrottleCheck.message };
  }

  recordOtpDispatch(targetPhone);
  clearDualFailedAttempts(clientIp, acctKey);
