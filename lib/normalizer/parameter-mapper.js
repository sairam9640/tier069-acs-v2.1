...
function normalizeDeviceData(rawParams = {}, baseDeviceId = {}, devContext = {}) {
  ...
  // 3. WiFi Configuration (2.4GHz & 5GHz) - auto-scan all WLAN indices
  const wifi24 = extractBestWifiDetails(params, [1, 3, 5], '2.4');
  const wifi5 = extractBestWifiDetails(params, [2, 4, 6], '5.0');

  // 4. WAN & PPPoE Configuration
  ...
  return {
    ...
    wifi: {
      wifi24,
      wifi5: (wifi5 && wifi5.ssid) ? wifi5 : null,
      isDualBand: !!(wifi5 && wifi5.ssid)
    },
    wan,
    network: {
      externalIP,
...
/**
 * Try multiple WLAN indices and return the first one with an SSID.
 * TP-Link typically uses indices 2-6; Syrotech uses 1-2; Genexis uses 1-4.
 */
function extractBestWifiDetails(params, indices, band) {
  for (const index of indices) {
    const details = extractWifiDetails(params, index, band);
    if (details && details.ssid) return details;
  }
  if (band === '5.0') return null;
  return extractWifiDetails(params, indices[0], band);
}

function extractWifiDetails(params, index = 1, band = '2.4') {
  ...
  const enabled = params[`${prefixTR098}Enable`] || params[`${prefixTR181}Enable`] || '1';
  const channel = params[`${prefixTR098}Channel`] || params[`Device.WiFi.Radio.${index}.Channel`] || 'Auto';
  const bssid = params[`${prefixTR098}BSSID`] || params[`${prefixTR181}BSSID`] || params[`${prefixTR098}MACAddress`] || '';
  const securityMode = params[`${prefixTR098}BeaconType`] || params[`${prefixTR098}Standard`] || params[`${apPrefixTR181}Security.ModeEnabled`] || 'WPA2-PSK (AES)';
  const txPower = params[`${prefixTR098}TransmitPower`] || params[`${prefixTR098}X_CT-COM_TxPower`] || '100%';
  const standard = band === '5.0' ? '802.11a/n/ac (5GHz)' : '802.11b/g/n (2.4GHz)';

  // WiFi Passphrase - try multiple variants & regex fallback
  ...
  const associatedDevices = params[`${prefixTR098}TotalAssociations`] ||
                            params[`${prefixTR098}AssociatedDeviceNumberOfEntries`] || '0';

  const bytesSent = params[`${prefixTR098}TotalBytesSent`] || params[`${prefixTR098}Stats.BytesSent`];
  const bytesReceived = params[`${prefixTR098}TotalBytesReceived`] || params[`${prefixTR098}Stats.BytesReceived`];

  // Determine param paths for writing back
  ...
  return {
    ...
    bssid,
    securityMode,
    transmitPower: txPower ? `${txPower}%` : '100%',
    standard,
    bytesSentFormatted: bytesSent ? formatBytes(bytesSent) : null,
    bytesReceivedFormatted: bytesReceived ? formatBytes(bytesReceived) : null,
...
function extractWanDetails(params) {
  ...
  for (const [k, v] of Object.entries(params)) {
    ...
    if (match) {
      ...
      if (!processedPaths.has(basePath)) {
        ...
        const enable = params[`${basePath}Enable`] || '1';
        const serviceList = params[`${connDeviceKey}X_CT-COM_ServiceList`] || params[`${basePath}X_CT-COM_ServiceList`] || 'INTERNET';
        const natEnabled = params[`${basePath}NATEnabled`] || '1';
        ...
        connections.push({
          ...
          natEnabled: natEnabled === '1' || natEnabled === 'true' || natEnabled === true,
  ...
  for (const [k, v] of Object.entries(params)) {
    ...
    if (match) {
      ...
      if (!processedPaths.has(basePath)) {
        ...
        const dnsServers = params[`${basePath}DNSServers`] || '';
        const enable = params[`${basePath}Enable`] || '1';
        const serviceList = params[`${connDeviceKey}X_CT-COM_ServiceList`] || params[`${basePath}X_CT-COM_ServiceList`] || 'TR069_VOIP';
        const natEnabled = params[`${basePath}NATEnabled`] || '1';
  ...
  // Find the primary internet connection (prefer PPPoE with username or active connection)
  let primary = connections.find(c => c.username && c.connectionType === 'PPPoE') ||
                connections.find(c => c.externalIP) ||
                connections[0] || {
                  connectionType: 'PPPoE',
                  username: '',
                  password: '',
                  ...
                  defaultGateway: '',
                  dnsServers: '',
                  paramPathUser: 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Username',
                  paramPathPass: 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Password',
                  paramPathVlan: 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.X_CT-COM_VLANIDMark'
                };

  return {
    connectionType: primary.connectionType || 'PPPoE',
    username: primary.username || '',
    password: primary.password || '',
    vlanId: primary.vlanId || 'Default / None',
    status: primary.status || 'Connected',
    ipAddress: primary.externalIP || '',
    defaultGateway: primary.defaultGateway || '',
    dnsServers: primary.dnsServers || '',
    paramPathUser: primary.paramPathUser || '',
    paramPathPass: primary.paramPathPass || '',
    paramPathVlan: primary.paramPathVlan || '',
...
function buildSmartParamList(supportedParams = []) {
  ...
  for (const name of supportedParams) {
    ...
    // WiFi SSID & Password (any WLAN index)
    if (/wlanconfiguration\.\d+\.(ssid|enable|channel|bssid|beacontype|totalassociations|associateddevicenumberofentries|keypassphrase|x_tp_presharedkey)$/i.test(name)) {
      wanted.add(name);
    }
    if (/wlanconfiguration\.\d+\.presharedkey\.1\.(keypassphrase|presharedkey)$/i.test(name)) {
      wanted.add(name);
    }
    // WAN PPPoE
    if (/wanpppconnection\.\d+\.(username|password|connectionstatus|externalipaddress|defaultgateway|dnsservers)$/i.test(name)) {
      wanted.add(name);
    }
    // WAN IP
    if (/wanipconnection\.\d+\.(externalipaddress|connectionstatus|macaddress)$/i.test(name)) {
      wanted.add(name);
    }
    // LAN Hosts
    if (/hosts\.host\.\d+\.(hostname|ipaddress|macaddress|active|interfacetype)$/i.test(name)) {
      wanted.add(name);
    }
    // MAC Address
    ...
    // VLAN
    if (/x_ct-com_vlanidmark$|vlanidmark$/i.test(name)) {
      wanted.add(name);
    }
  }
