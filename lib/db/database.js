...
async function saveSuperAdminOtpRecord({ email, challengeToken, otpHash, salt, expiresAt }) {
  const cleanEmail = String(email).toLowerCase().trim();
  const doc = {
    email: cleanEmail,
    challengeToken,
  ...
  if (!useLocalFallback && db) {
    try {
      ...
      await db.collection('superadmin_otps').updateMany(
        { email: cleanEmail, isUsed: false },
        { $set: { isUsed: true, invalidatedAt: new Date().toISOString() } }
      );
  ...
  memoryStore.superadminOtps.set(challengeToken, doc);
  return doc;
}
...
async function getSuperAdminOtpRecord(challengeToken) {
  ...
  if (!useLocalFallback && db) {
    try {
      return await db.collection('superadmin_otps').findOne({ challengeToken, isUsed: false });
    } catch (e) {
  ...
  return rec && !rec.isUsed ? rec : null;
}
