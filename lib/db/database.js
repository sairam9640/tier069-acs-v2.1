...
async function popPendingDeviceTask(deviceId) {
  ...
  // GenieACS Priority Processing: sort tasks so high-priority user actions (priority: 100) run before background discovery (priority: 10)
  dev.taskQueue.sort((a, b) => (b.priority || 10) - (a.priority || 10));
  ...
  dev.lastTaskStatus = {
    ...
    type: task.type,
    status: 'RUNNING',
    message: 'Delivering configuration parameters to ONT via TR-069 session'
  };
...
/**
 * Sweeps and expires tasks that have timed out without being picked up by router (Issue 2).
 */
async function expireStaleDeviceTasks(timeoutMs = 10 * 60 * 1000) {
  const now = Date.now();
  const allDevs = await getAllDevices();
  let expiredCount = 0;

  for (const dev of allDevs) {
    if (!dev.taskQueue || dev.taskQueue.length === 0) continue;
    let modified = false;

    const remainingTasks = [];
    for (const task of dev.taskQueue) {
      const createdTs = new Date(task.createdAt || 0).getTime();
      if (now - createdTs > timeoutMs) {
        expiredCount++;
        modified = true;
        await addLog({
          type: 'TASK_EXPIRED',
          deviceId: dev._id,
          message: `Task ${task.id} (${task.type}) expired after ${Math.round(timeoutMs/60000)}m timeout. Router unreachable.`
        });
        dev.lastTaskStatus = {
          id: task.id,
          type: task.type,
          status: 'EXPIRED',
          message: 'Task expired — router was unreachable over NAT during timeout window. Retry needed.',
...
// --- Billing / Radius Synchronization Queue (Issue 4) ---

async function queueBillingSync(syncRecord) {
  const item = {
    id: syncRecord.id || `BSYNC_${Date.now()}_${Math.floor(Math.random()*1000)}`,
    idempotencyKey: syncRecord.idempotencyKey || `sync_${syncRecord.deviceId}_${Date.now()}`,
    deviceId: syncRecord.deviceId,
    tenantId: syncRecord.tenantId || 'default',
    action: syncRecord.action || 'UPDATE_PPPOE',
    payload: syncRecord.payload || {},
    status: syncRecord.status || 'PENDING', // PENDING, RETRYING, COMPLETED, FAILED, UNCONFIGURED, NEEDS_MANUAL_REVIEW
    attempts: syncRecord.attempts || 0,
    maxAttempts: syncRecord.maxAttempts || 8,
    nextRetryAt: syncRecord.nextRetryAt || new Date().toISOString(),
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    error: syncRecord.error || null
  };

  memoryStore.billing_sync_queue.set(item.idempotencyKey, item);

  if (!useLocalFallback && db) {
    try {
      await db.collection('billing_sync_queue').updateOne(
        { idempotencyKey: item.idempotencyKey },
        { $set: item },
        { upsert: true }
      );
    } catch (e) {
      console.warn('Error queueing billing sync to DB:', e.message);
    }
  }
...
async function getPendingBillingSyncs(limit = 25) {
  const now = new Date().toISOString();
  if (!useLocalFallback && db) {
    try {
      return await db.collection('billing_sync_queue').find({
        status: { $in: ['PENDING', 'RETRYING', 'FAILED', 'UNCONFIGURED'] },
        $or: [
          { nextRetryAt: { $lte: now } },
  ...
  return Array.from(memoryStore.billing_sync_queue.values()).filter(item => {
    const isMatchingStatus = ['PENDING', 'RETRYING', 'FAILED', 'UNCONFIGURED'].includes(item.status);
    const isReadyForRetry = !item.nextRetryAt || item.nextRetryAt <= now;
    return isMatchingStatus && isReadyForRetry;
  }).slice(0, limit);
}

async function updateBillingSyncRecord(idempotencyKey, updates) {
  const ex = memoryStore.billing_sync_queue.get(idempotencyKey);
  if (ex) {
  ...
  if (!useLocalFallback && db) {
    try {
      ...
      console.warn('Error updating billing sync record:', e.message);
    }
  }
}

async function getBillingSyncStats() {
  if (!useLocalFallback && db) {
    try {
      const pending = await db.collection('billing_sync_queue').countDocuments({ status: { $in: ['PENDING', 'RETRYING'] } });
      const failed = await db.collection('billing_sync_queue').countDocuments({ status: 'FAILED' });
      const unconfigured = await db.collection('billing_sync_queue').countDocuments({ status: 'UNCONFIGURED' });
      const needsManualReview = await db.collection('billing_sync_queue').countDocuments({ status: 'NEEDS_MANUAL_REVIEW' });
      const completed = await db.collection('billing_sync_queue').countDocuments({ status: 'COMPLETED' });
      return { pending, failed, unconfigured, needsManualReview, completed, stuckTotal: failed + unconfigured + needsManualReview };
    } catch (e) {
      // Fallback
    }
  }

  const all = Array.from(memoryStore.billing_sync_queue.values());
  const pending = all.filter(i => ['PENDING', 'RETRYING'].includes(i.status)).length;
  const failed = all.filter(i => i.status === 'FAILED').length;
  const unconfigured = all.filter(i => i.status === 'UNCONFIGURED').length;
  const needsManualReview = all.filter(i => i.status === 'NEEDS_MANUAL_REVIEW').length;
...
async function getTenant(idOrSlug) {
  ...
  return tenants.find(t => {
    ...
    const tDomain = (t.domain || '').trim().toLowerCase();
    ...
    return tSlug === cleanId ||
           ...
           tUser === cleanId ||
           tDomain === cleanId ||
           (tPhone && cleanDigits.length >= 10 && (tPhone === cleanDigits || tPhone.endsWith(cleanDigits)));
  }) || null;
}
...
module.exports = {
  ...
  getAuditLogs,
  getOtpLogs,
  expireStaleDeviceTasks,
  queueBillingSync,
  getPendingBillingSyncs,
  updateBillingSyncRecord,
  getBillingSyncStats
};
