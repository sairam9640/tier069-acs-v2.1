...
function createApiRouter(eventBus) {
  ...
  // --- OPERATOR DEVICE VERIFICATION / QUARANTINE RELEASE ENDPOINT ---
  router.post('/devices/:id/verify', async (req, res) => {
    try {
      const dev = await db.getDevice(req.params.id);
      if (!dev) return res.status(404).json({ success: false, error: 'Device not found' });
      if (!checkDeviceTenantAccess(req.user, dev)) {
        return res.status(403).json({ success: false, error: 'Forbidden: Cannot verify device from another operator fleet' });
      }
      ...
      dev.verifiedBy = req.user?.username || 'operator';

      await db.saveDevice(dev);
      if (eventBus) eventBus.emit('device_updated', dev);

      await db.addAuditLog({
        event: 'DEVICE_QUARANTINE_RELEASED',
        deviceId: dev._id,
        operator: req.user?.username,
        tenantId: dev.tenantId
      });

      res.json({ success: true, message: `Device ${dev._id} verified and approved as active subscriber.`, device: dev });
    } catch (err) {
      res.status(500).json({ success: false, error: err.message });
    }
  });
  ...
  router.post('/billing/sync', express.json(), async (req, res) => {
    try {
      ...
      if (!secret) {
        return res.status(503).json({ error: 'Billing sync engine unconfigured (BILLING_WEBHOOK_SECRET missing in .env)' });
      }
  ...
  router.post('/devices/:id/wan/delete', async (req, res) => {
    try {
      ...
      // Clean from memory immediately so UI updates
      if (dev.wan && Array.isArray(dev.wan.connections)) {
        dev.wan.connections = dev.wan.connections.filter(c => c.path !== wanPath && !wanPath.startsWith(c.path));
      }
      dev.lastConfigModified = Date.now();
      dev.configVersion = (dev.configVersion || 0) + 1;
      await db.saveDevice(dev);
  ...
  router.post('/devices/:id/refresh', async (req, res) => {
    try {
      ...
      const msg = triggerRes.success
        ? `Direct wake-up signal sent to ONT! Fetching fresh parameters now. (${triggerRes.message})`
        : `Summon sync task queued with Priority 100! Router is behind NAT — will sync on next keepalive check-in.`;
  ...
  router.post('/devices/:id/ping', async (req, res) => {
    try {
      ...
      // Strict IP address validation to prevent command injection
      ...
      const pingArgs = isWin ? ['-n', '4', targetIp] : ['-c', '4', '-W', '2', targetIp];

      execFile('ping', pingArgs, { timeout: 8000 }, (error, stdout, stderr) => {
