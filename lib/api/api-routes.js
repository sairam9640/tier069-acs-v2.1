  ...
  router.post('/billing/sync', express.json(), async (req, res) => {
    try {
      ...
      await db.addLog({
        ...
        deviceId: req.body.deviceId,
        message: `Radius AAA Webhook accepted update for subscriber "${req.body.username}" (VLAN ${req.body.vlanId}) via HMAC auth`
      });

      return res.status(200).json({
        success: true,
        status: 'ACCEPTED',
        message: `Radius AAA subscriber database synchronized for ${req.body.username}`,
        receipt
      });
    } catch (err) {
