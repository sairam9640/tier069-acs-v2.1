...
function createApiRouter(eventBus) {
  ...
  router.get('/devices/summary', async (req, res) => {
    try {
      ...
      const activeTenant = req.user?.role === 'OPERATOR' ? req.user.tenantId : (req.query.tenant || req.query.tenantId || null);
      if (activeTenant && activeTenant !== 'all') {
        devices = devices.filter(d => (d.tenantId || 'rudra').toLowerCase() === activeTenant.toLowerCase());
      }

      const onlineCutoff = Date.now() - 20 * 60 * 1000;
      let online = 0;
      let offline = 0;
      let critical = 0;
      let sumRx = 0;
  ...
  router.get('/devices', async (req, res) => {
    try {
      ...
      const activeTenant = req.user?.role === 'OPERATOR' ? req.user.tenantId : (tenant || tenantId || null);
      if (activeTenant && activeTenant !== 'all') {
        dbDevices = dbDevices.filter(d => (d.tenantId || 'rudra').toLowerCase() === activeTenant.toLowerCase());
      }

      // Fetch all real ONUs discovered on physical Syrotech OLT (Rudra & SuperAdmin only)
      let oltOnus = [];
      const isRudraOrSA = !activeTenant || activeTenant === 'all' || activeTenant.toLowerCase() === 'rudra';
      if (isRudraOrSA) {
        try {
  ...
  router.post(['/devices/create', '/customer/provision'], async (req, res) => {
    try {
      ...
      const newDevice = {
        ...
        oltName: oltName || 'SyroTech OLT-01',
        oltIp: '172.18.18.3',
        ponPort: ponPort || 'EPON 0/1',
        oltPort: ponPort || 'EPON 0/1',
        location: {
          lat: latitude,
          lng: longitude,
          address: address || '',
  ...
  router.post('/devices/:id/wifi', async (req, res) => {
    try {
      ...
      const { band, ssid, password, channel } = req.body;
      const wifiObj = band === '5G' ? dev.wifi?.wifi5 : dev.wifi?.wifi24;

      const paramPathSSID = wifiObj?.paramPathSSID || (band === '5G' ? 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.SSID' : 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.SSID');
      const paramPathPassword = wifiObj?.paramPathPassword || (band === '5G' ? 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.PreSharedKey.1.KeyPassphrase' : 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.PreSharedKey.1.KeyPassphrase');
      ...
      const task = await db.queueDeviceTask(dev._id, {
        ...
        paramPathSSID,
        paramPathPassword,
        paramPathChannel: (band === '5G' ? 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.Channel' : 'InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.Channel')
      });

      if (dev.wifi) {
        if (band === '5G' && dev.wifi.wifi5) {
          if (ssid) dev.wifi.wifi5.ssid = ssid;
          if (password) dev.wifi.wifi5.password = password;
        } else if (dev.wifi.wifi24) {
          if (ssid) dev.wifi.wifi24.ssid = ssid;
          if (password) dev.wifi.wifi24.password = password;
        }
        await db.saveDevice(dev);
      ...
      await db.addLog({
        type: 'WIFI_UPDATE',
        deviceId: dev._id,
        message: `Queued WiFi update (${band || '2.4G'}) -> SSID: "${ssid}"`
      });
      ...
      const msg = triggerRes.success
        ? `WiFi settings pushed to ONT! (${triggerRes.message})`
        : `WiFi update queued! ${triggerRes.message}`;
  ...
  router.post('/devices/:id/wan', async (req, res) => {
    try {
      ...
      const { username, password, vlanId } = req.body;
      ...
      const task = await db.queueDeviceTask(dev._id, {
        ...
        password,
        vlanId,
        paramPathUser: dev.wan?.paramPathUser || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Username',
        paramPathPass: dev.wan?.paramPathPass || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Password',
        paramPathVlan: dev.wan?.paramPathVlan || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.X_CT-COM_VLANIDMark'
      });

      if (dev.wan) {
        if (username) dev.wan.username = username;
        if (password) dev.wan.password = password;
        if (vlanId) dev.wan.vlanId = vlanId;
        await db.saveDevice(dev);
      }
      ...
      await db.addLog({
        type: 'WAN_UPDATE',
        deviceId: dev._id,
        message: `Queued WAN PPPoE change: User "${username}", VLAN: ${vlanId || 'Default'}`
      });
      ...
      const msg = triggerRes.success
        ...
        : `WAN credentials queued! ${triggerRes.message}`;
  ...
  router.post('/devices/:id/wan/add', async (req, res) => {
    try {
      ...
      const { connectionType, serviceList, vlanId, username, password, enableNat } = req.body;
      const nextIndex = (dev.wan?.connections?.length || 1) + 1;
      const isPppoe = connectionType !== 'Bridge' && connectionType !== 'IP_Routed';
      const paramsToSet = [];

      if (isPppoe) {
        const basePath = `InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${nextIndex}.WANPPPConnection.1.`;
        ...
        paramsToSet.push({ name: `${basePath}Password`, value: password || 'pass', type: 'xsd:string' });
        ...
        const basePath = `InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${nextIndex}.WANIPConnection.1.`;
        const connDev = `InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${nextIndex}.`;
        paramsToSet.push({ name: `${basePath}Enable`, value: '1', type: 'xsd:boolean' });
        paramsToSet.push({ name: `${basePath}ConnectionType`, value: connectionType === 'Bridge' ? 'Bridge' : 'IP_Routed', type: 'xsd:string' });
        if (vlanId) {
          paramsToSet.push({ name: `${connDev}X_CT-COM_VLANIDMark`, value: String(vlanId), type: 'xsd:unsignedInt' });
        }
      }
      ...
      await db.addLog({
        type: 'WAN_ADD',
        deviceId: dev._id,
        message: `Queued new ${connectionType || 'PPPoE'} WAN creation (VLAN: ${vlanId || 'None'}, Service: ${serviceList || 'INTERNET'})`
      });
      ...
      const msg = triggerRes.success
        ? `New WAN profile (${connectionType || 'PPPoE'}) pushed to ONT! (${triggerRes.message})`
        : `New WAN profile queued! Will apply on next ONT check-in.`;
  ...
  router.post('/superadmin/tenants', async (req, res) => {
    try {
      ...
      const existing = await db.getTenant(cleanSlug);
      if (existing) {
        return res.status(400).json({ success: false, message: `An operator with slug "${cleanSlug}" already exists` });
      }
