  ...
  <div class="card">
    ...
    <p style="color:#64748b;font-size:0.85rem;">Target Operator: <strong style="color:#e2e8f0;">${tenantName}</strong> | Tenant Slug: <strong style="color:#38bdf8;">${tenantSlug}</strong></p>
      ...
      if (messageType === 'Inform') {
        ...
        return await handleDeleteObjectResponse(session, parsed.status, cwmpId, res, eventBus);
      } else if (messageType === 'AddObjectResponse') {
        return await handleAddObjectResponse(session, parsed.instanceNumber, parsed.status, cwmpId, res, eventBus);
...
// Helper: Build standard TR-069 InformResponse XML envelope
function buildMinimalInformResponse(cwmpId = '1') {
...
<soap-env:Envelope 
  ...
  <soap-env:Header>
    <cwmp:ID soap-env:mustUnderstand="1">${cwmpId || '1'}</cwmp:ID>
...
// Quarantine Creation Rate Limiter (Max 5 new unverified devices per source IP per hour)
const quarantineCreationThrottle = new Map(); // clientIp -> { count, firstTs }
const MAX_QUARANTINE_PER_IP_PER_HOUR = 5;
const QUARANTINE_WINDOW_MS = 60 * 60 * 1000; // 1 hour window

function checkQuarantineCreationLimit(clientIp) {
  const now = Date.now();
  const cleanIp = (clientIp || '').trim();
  let entry = quarantineCreationThrottle.get(cleanIp);
  if (!entry || now - entry.firstTs > QUARANTINE_WINDOW_MS) {
    return { allowed: true };
  }
  if (entry.count >= MAX_QUARANTINE_PER_IP_PER_HOUR) {
    return { allowed: false, count: entry.count };
  }
  return { allowed: true };
}

function recordQuarantineCreation(clientIp) {
  const now = Date.now();
  const cleanIp = (clientIp || '').trim();
  let entry = quarantineCreationThrottle.get(cleanIp);
  if (!entry || now - entry.firstTs > QUARANTINE_WINDOW_MS) {
  ...
  quarantineCreationThrottle.set(cleanIp, entry);
}
...
async function handleInform(session, informData, cwmpId, clientIp, res, eventBus) {
  try {
    if (!informData || !informData.deviceId) {
      ...
      return res.status(200).send(buildMinimalInformResponse(cwmpId));
    }
    ...
    if (!rawSn || rawSn === 'UNKNOWN' || rawSn.length < 4 || rawSn.length > 64 || !/^[a-zA-Z0-9_-]+$/.test(rawSn)) {
      console.warn(`[CWMP REJECT] ❌ Implausible or malformed device serial number received: "${rawSn}". Acknowledging Inform without DB persistence.`);
      res.setHeader('Content-Type', 'text/xml; charset=utf-8');
      return res.status(200).send(buildMinimalInformResponse(cwmpId));
    }
    ...
    if (!existingDevice) {
      // Check Quarantine creation rate limiter per source IP (Max 5/hour)
      const throttleCheck = checkQuarantineCreationLimit(clientIp);
      if (!throttleCheck.allowed) {
        console.warn(`[CWMP THROTTLE] 🛑 Source IP ${clientIp} exceeded max quarantine limit (${MAX_QUARANTINE_PER_IP_PER_HOUR}/hour). Dropping device creation.`);
        res.setHeader('Content-Type', 'text/xml; charset=utf-8');
        return res.status(200).send(buildMinimalInformResponse(cwmpId));
      }

      recordQuarantineCreation(clientIp);
      isNewDevice = true;
      existingDevice = {
        _id: deviceId,
    ...
    const updatedDevice = {
      ...
      isVerified: (isNewDevice && existingDevice.isVerified === false) ? false : true,
      quarantined: (isNewDevice && existingDevice.isVerified === false) ? true : false,
      quarantineReason: (isNewDevice && existingDevice.isVerified === false) ? (existingDevice.quarantineReason || 'Auto-Discovered from CWMP: Awaiting Operator Verification') : null,
      status: (isNewDevice && existingDevice.isVerified === false) ? 'UNVERIFIED' : 'online',
      ipAddress: clientIp,
      network: {
        ...normalized.network,
...
async function handleEmptyRequest(session, res, eventBus) {
  try {
    ...
    if (task.type === 'SET_WIFI') {
      ...
      if (task.password) {
        // Resolve exact single passphrase path based on discovered device parameters (GenieACS precision)
        let passPath = task.paramPathPassword;
        if (!passPath || passPath.includes('KeyPassphrase')) {
          const rawKeys = Object.keys(dev?.rawParameters || {});
          if (rawKeys.some(k => k === `${baseWlanPath}PreSharedKey.1.KeyPassphrase`)) {
            passPath = `${baseWlanPath}PreSharedKey.1.KeyPassphrase`;
          } else if (rawKeys.some(k => k === `${baseWlanPath}KeyPassphrase`)) {
            passPath = `${baseWlanPath}KeyPassphrase`;
          } else if (rawKeys.some(k => k === `${baseWlanPath}X_TP_PreSharedKey`)) {
            passPath = `${baseWlanPath}X_TP_PreSharedKey`;
          } else {
            passPath = `${baseWlanPath}PreSharedKey.1.KeyPassphrase`;
          }
        }
      ...
      const dev = await db.getDevice(deviceId);
      const paramsToSet = [];
      const basePath = task.wanPath || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.';

      let userPath = task.paramPathUser || `${basePath}Username`;
      let passPath = task.paramPathPass || `${basePath}Password`;

      if (task.username !== undefined) {
        paramsToSet.push({ name: userPath, value: task.username, type: 'xsd:string' });
      }
      if (task.password !== undefined && task.password !== '') {
        paramsToSet.push({ name: passPath, value: task.password, type: 'xsd:string' });
      }
      if (task.vlanId !== undefined && task.vlanId !== 'None' && task.vlanId !== 'Untagged' && task.vlanId !== '' && task.vlanId !== '0') {
        ...
        const vlanPath = task.paramPathVlan || `${connDeviceKey}X_CT-COM_VLANIDMark`;
        paramsToSet.push({ name: vlanPath, value: String(task.vlanId), type: 'xsd:unsignedInt' });
      }
      ...
      const objectPath = task.objectPath || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.';
      const xml = buildAddObject(msgId, objectPath);
      console.log(`[CWMP] Sending AddObject (${objectPath}) to ${deviceId}`);
      return res.status(200).send(xml);
...
async function handleDeleteObjectResponse(session, status, cwmpId, res, eventBus) {
  try {
    const deviceId = session.deviceId;
    const task = session.activeTask;
    console.log(`[CWMP] DeleteObject completed on ${deviceId} (Status: ${status})`);

    const dev = await db.getDevice(deviceId);
    if (dev && task && task.objectPath) {
      if (dev.wan && Array.isArray(dev.wan.connections)) {
        dev.wan.connections = dev.wan.connections.filter(c => c.path !== task.objectPath && !task.objectPath.startsWith(c.path));
      }
      dev.lastTaskStatus = {
        id: task.id,
        type: 'DELETE_WAN',
        status: 'COMPLETED',
        message: `WAN profile [${task.objectPath}] deleted from router hardware.`,
        updatedAt: new Date().toISOString()
      };
      await db.saveDevice(dev);
    ...
    await db.addLog({
      type: 'WAN_DELETED',
      deviceId,
      message: `DeleteObject confirmed on ${deviceId} for path "${task?.objectPath || 'WAN'}" (Status ${status})`
    });

    session.activeTask = null;
    return await handleEmptyRequest(session, res, eventBus);
  } catch (err) {
    console.error('[DELETE OBJECT RESP ERROR]', err);
    return res.status(204).end();
  }
}

async function handleAddObjectResponse(session, instanceNumber, status, cwmpId, res, eventBus) {
  try {
    const deviceId = session.deviceId;
    const task = session.activeTask;
    console.log(`[CWMP] AddObject completed on ${deviceId}: Instance ${instanceNumber} created (Status: ${status})`);

    const dev = await db.getDevice(deviceId);
    if (dev && task && task.type === 'ADD_WAN') {
      const basePath = task.objectPath || `InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.${instanceNumber}.`;
      const paramsToSet = [];
      if (task.username) {
        paramsToSet.push({ name: `${basePath}Username`, value: task.username, type: 'xsd:string' });
      }
      if (task.password) {
        paramsToSet.push({ name: `${basePath}Password`, value: task.password, type: 'xsd:string' });
      }
      if (task.vlanId) {
        const connDeviceKey = basePath.substring(0, basePath.indexOf('WANPPPConnection')) || basePath.substring(0, basePath.indexOf('WANIPConnection')) || 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.';
        paramsToSet.push({ name: `${connDeviceKey}X_CT-COM_VLANIDMark`, value: String(task.vlanId), type: 'xsd:unsignedInt' });
      }
      ...
      dev.lastTaskStatus = {
        ...
        type: 'ADD_WAN',
        status: 'COMPLETED',
        message: `WAN instance ${instanceNumber} added on hardware. Parameters queued for push.`,
        updatedAt: new Date().toISOString()
      };
      await db.saveDevice(dev);
    }
    ...
    console.error('[ADD OBJECT RESP ERROR]', err);
    return res.status(204).end();
  }
}
