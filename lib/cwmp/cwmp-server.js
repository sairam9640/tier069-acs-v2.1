...
<body>
  <div class="card">
    ...
    <h1>VRV ACS Cloud CWMP Server</h1>
    <p>TR-069 / CWMP Port 7547 is active, running, and accepting SOAP Inform packets from customer ONTs.</p>
    <div class="url-box">${tenantSlug === 'rudra' ? 'http://222.167.207.220:7547/' : `http://${tenantSlug}.ciniplay.in:7547/`}</div>
    ${tenantSlug !== 'rudra' ? `<div style="font-size:0.75rem;color:#94a3b8;margin-bottom:1rem;" class="mono">🔒 Secure URL: https://${tenantSlug}.ciniplay.in:7547/<br>Alternate IP: http://222.167.207.220:7547/${tenantSlug}</div>` : ''}
...
async function handleInform(session, informData, cwmpId, clientIp, res, eventBus) {
  try {
    ...
    const cleanSn = safeStr(serialNumber || oui, 'UNKNOWN').replace(/[^a-zA-Z0-9_-]/g, '_');
...
async function handleEmptyRequest(session, res, eventBus) {
  try {
    ...
    if (task.type === 'SET_WIFI') {
      const paramsToSet = [];
      if (task.paramPathSSID && task.ssid) {
        paramsToSet.push({ name: task.paramPathSSID, value: task.ssid, type: 'xsd:string' });
      }
      if (task.paramPathPassword && task.password) {
        paramsToSet.push({ name: task.paramPathPassword, value: task.password, type: 'xsd:string' });
      }
      if (task.channel && task.paramPathChannel) {
        paramsToSet.push({ name: task.paramPathChannel, value: task.channel, type: 'xsd:unsignedInt' });
      }
      const xml = buildSetParameterValues(msgId, paramsToSet);
      console.log(`[CWMP] Sending SetParameterValues (WiFi) to ${deviceId}`);
      return res.status(200).send(xml);

    } else if (task.type === 'SET_WAN') {
      const paramsToSet = [];
      if (task.paramPathUser && task.username) {
        paramsToSet.push({ name: task.paramPathUser, value: task.username, type: 'xsd:string' });
      }
      if (task.paramPathPass && task.password) {
        paramsToSet.push({ name: task.paramPathPass, value: task.password, type: 'xsd:string' });
      }
      if (task.paramPathVlan && task.vlanId) {
        paramsToSet.push({ name: task.paramPathVlan, value: task.vlanId, type: 'xsd:unsignedInt' });
      }
      const xml = buildSetParameterValues(msgId, paramsToSet);
      console.log(`[CWMP] Sending SetParameterValues (WAN) to ${deviceId}`);
      return res.status(200).send(xml);
