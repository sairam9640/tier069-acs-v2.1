...
async function syncBillingWanChange(deviceId, wanData, configVersion = 1, customWebhookUrl = null, customSecret = null) {
  ...
  const customerUser = String(wanData?.username || wanData?.pppoeUsername || wanData?.account || 'unknown_subscriber').trim();

  const payload = {
    idempotencyKey,
    deviceId,
    username: customerUser,
    vlanId: wanData?.vlanId !== undefined ? wanData.vlanId : 0,
  ...
  if (!targetUrl || !targetSecret) {
    ...
    await db.addLog({
      ...
      deviceId,
      message: `Billing/Radius sync skipped for [${customerUser}]: ${unconfigMsg}`
    });

    return { success: false, configured: false, error: unconfigMsg, idempotencyKey };
  }

  // Loopback blocking in production check
  ...
  const allowLoopback = process.env.ALLOW_LOOPBACK_BILLING === 'true';

  if (isLoopback && !allowLoopback) {
    const blockMsg = `Security policy: Loopback/Private IP webhook (${targetUrl}) refused in production. Set ALLOW_LOOPBACK_BILLING=true to allow for testing.`;
    console.error(`🛑 [SECURITY REFUSAL] ${blockMsg}`);
    ...
    await db.queueBillingSync({
      ...
      action: 'UPDATE_PPPOE_WAN',
      payload,
      status: 'BLOCKED_LOOPBACK_IN_PRODUCTION',
      error: blockMsg
    });
    ...
    if (dev) {
      if (!dev.wan) dev.wan = {};
      dev.wan.billingSynced = false;
      dev.wan.billingSyncError = blockMsg;
      await db.saveDevice(dev);
    }

    await db.addLog({
      type: 'BILLING_SYNC_BLOCKED',
      deviceId,
      message: blockMsg
    });
...
async function testBillingConnection(targetUrl = null, secret = null) {
  ...
  const pingPayload = {
    ...
    username: 'admin_health_check',
    deviceId: 'ACS_SYSTEM_TEST',
    timestamp: Date.now()
  };
