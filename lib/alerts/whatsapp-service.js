...
const SESSIONS_BASE_DIR = process.env.WHATSAPP_SESSIONS_DIR || 
  (fs.existsSync('/opt/tr069-acs/data/wa_sessions') ? '/opt/tr069-acs/data/wa_sessions' :
   (fs.existsSync('/opt/tr069-acs/whatsapp_sessions') ? '/opt/tr069-acs/whatsapp_sessions' :
...
class WhatsAppService {
  ...
  async processQueue() {
    ...
    try {
      ...
      let jid = item.phone;
      if (!jid.includes('@')) {
        let clean = String(item.phone).replace(/\D/g, '').replace(/^0+/, '');
  ...
  async sendSuperAdminLoginOtp(phone, otp) {
    const targetPhone = String(phone || process.env.SUPER_ADMIN_PHONE || '').replace(/\D/g, '').replace(/^0+/, '');
